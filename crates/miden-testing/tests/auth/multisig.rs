...
async fn test_executor_bug() -> anyhow::Result<()> {
    ...
    let sig_1 = authenticators[0].get_signature(public_keys[0].clone().into(), &tx_summary).await?;
    let sig_2 = authenticators[1].get_signature(public_keys[1].clone().into(), &tx_summary).await?;
    ...
    // The block below demonstrates the bug: when using unauthenticated notes directly,
    // the transaction fails with Unauthorized error even though signatures are provided.
    //---------------------------------------------------------------------------------------------
    let block_num = tx_context_with_signatures.tx_inputs().block_header().block_num();
    let tx_args = tx_context_with_signatures.tx_args().clone();
    ...
    // Test 1: Using notes from the context (should work)
    println!("Test 1: Using notes from context (authenticated)...");
    let notes_from_context = tx_context_with_signatures.tx_inputs().input_notes().clone();
    ...
    let result_with_context_notes = tx_executor
        .execute_transaction(multisig_account.id(), block_num, notes_from_context, tx_args.clone())
        .await;
    
    match &result_with_context_notes {
        Ok(_) => println!("✓ Success with context notes (as expected)"),
        Err(e) => println!("✗ Failed with context notes: {:?}", e),
    }

    // Test 2: Using manually created unauthenticated notes (demonstrates the bug)
    println!("\nTest 2: Using manually created unauthenticated notes...");
    let unauthenticated_notes = vec![input_note].into();
    ...
    let result_with_unauth_notes = tx_executor
        .execute_transaction(multisig_account.id(), block_num, unauthenticated_notes, tx_args)
        .await;
    
    match &result_with_unauth_notes {
        Ok(_) => println!("✓ Success with unauthenticated notes"),
        Err(e) => println!("✗ Failed with unauthenticated notes: {:?}", e),
    }

    // The bug: Transaction fails with Unauthorized error when using unauthenticated notes,
    // even though the required signatures are provided. This suggests that the authorization
    // logic is incorrectly coupled with note authentication status.
    println!("\n=== BUG DEMONSTRATION ===");
    println!("The transaction fails with 'Unauthorized' error when using unauthenticated notes,");
    println!("despite having valid signatures from both required approvers.");
    println!("This indicates that authorization (signatures) is incorrectly coupled with");
    println!("note authentication (inclusion proofs).");
    
    // Assert the bug exists
    assert!(result_with_context_notes.is_ok(), "Context notes should work");
    assert!(result_with_unauth_notes.is_err(), "Unauthenticated notes currently fail (bug)");
    
    if let Err(TransactionExecutorError::Unauthorized(_)) = result_with_unauth_notes {
        println!("\n✗ Confirmed: Unauthorized error with valid signatures (BUG)");
    }
